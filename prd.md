...
- **상세**: 첫 번째 세트에서 수정된 무게와 횟수 데이터를 AI가 인식하여, 남은 모든 세트에 즉시 적용함. Zero-Click 세트 동기화]

### [F2] 0.1s 단위 AI 박자 가이드 (Tempo Engine)
- **상세**: 설정된 주기에 맞춰 "One, Two" 음성을 실시간 출력하며 시각적 카운트다운 제공. 0.1s 단위 AI 박자 가이드]

### [F3] 빅 스텝퍼 UI & 자동 확정 (Interaction)
- **상세**: 수치 클릭 시 대형 버튼 노출. 상단에 무게/횟수/시간 모드 전환 탭 제공. 빅 스텝퍼 UI]

### [F4] 중급자용 스마트 증량 로직 (Progression)
- **상세**: 마지막 성공 데이터로부터 14일 경과 시 AI가 무게 5kg 상향을 자동 제안 및 반영함. 중급자용 스마트 증량 로직]

### **[F5] [신규] 자동화 테스트 지원 (Automated Testing)**
- **상세**: 핵심 비즈니스 로직(증량, 동기화)에 대한 Unit Test 스크립트를 포함하여 코드 무결성 보장.
...
- **Step 3 (테스트)**: **[신규] [F5]에 명시된 성공 조건을 검증하는 자동 테스트 스크립트 실행.**
